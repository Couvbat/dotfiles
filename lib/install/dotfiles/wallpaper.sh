...
if [ -d $HOME/.config/waypaper ]; then
    ...
    if [ -d ~/wallpaper/ ]; then
        ...
        echo
        _writeMessage "You can download and install additional wallpapers from https://github.com/Couvbat/wallpaper/"
        echo
        if gum confirm "Do you want to download the repository?"; then
            ...
            git clone --depth 1 https://github.com/Couvbat/wallpaper.git $download_folder/wallpaper
